...
export function detectDecisions(messages: MessageRow[], sessionId: string): DecisionRow[] {
  const decisions: DecisionRow[] = [];
  const sorted = [...messages].sort((a, b) => a.sequence_number - b.sequence_number);

  let prevAssistant: MessageRow | null = null;
  ...
  for (const msg of sorted) {
    ...
    const toolUses = prevAssistant?.tool_uses
      ? JSON.parse(prevAssistant.tool_uses) as Array<{ name: string; input: Record<string, unknown> }>
      : null;

    const result = classifyDecision(
      msg.content,
      prevAssistant?.content ?? null,
      toolUses,
    );
