...
import { loadConfig, updateConfig, type PromptUpConfig } from './config.js';
 ...
 */
function readSessionEndJson(): Record<string, unknown> | null {
  try {
    const candidates = [
 ...
 */
function parseToolEventsForGit(sessionId: string): void {
  const candidates = [
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        try {
          ...
          if (event.type !== 'tool_use' && event.type !== 'assistant') continue;
          ...
          if (event.name === 'Bash' && event.input && typeof event.input === 'object') {
            toolUses.push({ name: 'Bash', input: event.input as Record<string, unknown> });
            ...
            for (const block of event.content as Array<Record<string, unknown>>) {
              if (block.type === 'tool_use' && block.name === 'Bash' && block.input) {
                toolUses.push({ name: 'Bash', input: block.input as Record<string, unknown> });
              }
            }
          }
...
export async function handleEvaluateSession(args: {
  ...
  try {
    ...
    if (!transcriptPath) {
      return textResponse(
        'No transcript found. Ensure Claude Code is running and has an active session, ' +
        'or provide a session_id for a previously tracked session.\n\n' +
        'Transcripts are expected at ~/.claude/projects/<hash>/<session>.jsonl',
        true,
      );
    }
    ...
    if (messages.length === 0) {
      return textResponse(
        `Transcript at ${transcriptPath} contains no user/assistant messages.`,
        true,
      );
    }
    ...
    if (!evaluation) {
      return textResponse(
        'Evaluation failed. Claude Code CLI may not be available, or the session is too short ' +
    ...
    // Parse eval data
    const dimScores = JSON.parse(evaluation.dimension_scores) as Array<{
      key: string;
      score: number;
      weight?: number;
      reasoning?: string;
    ...
    const recommendations = evaluation.recommendations
      ? JSON.parse(evaluation.recommendations) as Array<{
    ...
    const trends = evaluation.trends
      ? JSON.parse(evaluation.trends) as Array<{
          dimension_key: string;
          direction: string;
          delta: number;
        }>
    ...
    for (const t of trends) {
      if (t.direction === 'improving') trendMap.set(t.dimension_key, ` ▲+${Math.abs(t.delta)}`);
      else if (t.direction === 'declining') trendMap.set(t.dimension_key, ` ▼${t.delta}`);
    }
    ...
    lines.push(
      '',
      `Developer prompts: **${userCount}** | Claude responses: **${assistantCount}**`,
    );
...
export async function handleGeneratePRReport(args: {
  ...
  try {
        ...
        if (existing.length === 0) {
          try {
            ...
            if (msgs.length > 0) {
              ...
              for (const m of msgs) m.session_id = s.id;
    ...
    if (report.repo) text += `**Repo:** ${report.repo}\n`;
    if (report.pr_url) text += `**PR:** ${report.pr_url}\n`;
    text += `**DQS:** ${report.dqs !== null ? `${report.dqs}/100` : 'N/A'}\n`;
    text += `**Decisions:** ${report.total_decisions}\n`;
    if (report.posted_at) text += `**Posted to PR:** ${report.posted_at}\n`;
...
export async function handleGetStatus(_args: {}): Promise<ToolResponse> {
  try {
    ...
    if (sessionEnd) {
      ...
      if (sessionEnd.session_id) text += `- **Session ID:** ${sessionEnd.session_id}\n`;
      if (sessionEnd.transcript_path) text += `- **Transcript:** ${sessionEnd.transcript_path}\n`;
      if (sessionEnd.project_path) text += `- **Project:** ${sessionEnd.project_path}\n`;
      text += '\n';
    }

    // Latest evaluation
    if (latestEval) {
      const dimScores = JSON.parse(latestEval.dimension_scores) as Array<{
        key: string;
        score: number;
      }>;
...
function formatConfig(config: PromptUpConfig): string {
  const lines = [
    ...
    `| Band | Range |`,
    `|------|-------|`,
    ...Object.entries(config.classification.bands).map(
...
function getNestedFromConfig(obj: any, path: string): unknown {
