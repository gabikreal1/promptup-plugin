...
import {
  ...
  BASE_DIMENSION_KEYS,
  DOMAIN_DIMENSIONS,
  DOMAIN_DIMENSION_KEYS,
  WEIGHT_PROFILES,
  type BaseDimensionKey,
  type DomainDimensionKey,
} from './shared/dimensions.js';

import {
  computeCompositeScore,
  computeDomainComposite,
  computeTechComposite,
  computeOverallComposite,
  computeGrandComposite,
  computeRiskFlagsWithHistory,
...
// ─── Roadmap Types (inline — no shared roadmaps file in plugin) ──────────
 ...
 */
const ALL_ROADMAPS: Record<string, RoadmapDefinition> = {
  // Role roadmaps
...
function buildEvalPrompt(messages: MessageRow[]): string {
    ...
    const tools = m.tool_uses ? (() => {
      try {
        ...
        if (Array.isArray(parsed)) {
          return ` [tools: ${parsed.map((t: { name?: string }) => t.name || '?').join(', ')}]`;
        }
      } catch { /* ignore */ }
      return '';
    })() : '';
...
interface ClaudeEvalResult {
  dimensions: { key: string; score: number; reasoning: string }[];
  domain_dimensions?: { key: string; score: number; reasoning: string }[];
  tech_expertise?: { roadmap: string; score: number; competencies?: Record<string, number> }[];
  recommendations: { dimension_key: string; priority: string; recommendation: string; suggestions?: string[] }[];
  activity_log?: string[];
  decisions?: { type: string; summary: string; signal: string }[];
}

function runClaudeCode(prompt: string, timeoutMs = 120_000): Promise<string> {
...
function parseClaudeResponse(raw: string): ClaudeEvalResult {
  ...
  return parsed as ClaudeEvalResult;
}
...
export async function evaluateSession(
  ...
  let dimensionScores: EvalDimensionScore[];
  let domainDimensionScores: EvalDimensionScore[] = [];
  let techExpertise: TechExpertiseEntry[] = [];
  let rawEvaluation: string | null = null;
  let recommendations: EvalRecommendation[] = [];
  ...
  try {
    ...
    if (result.tech_expertise && result.tech_expertise.length > 0) {
      techExpertise = result.tech_expertise
        ...
        .map(te => {
          ...
          const competencies: Record<string, CompetencyScore> = {};
    ...
    recommendations = (result.recommendations || []).slice(0, 3).map(r => ({
      dimension_key: r.dimension_key as BaseDimensionKey,
    ...
    console.warn(`[eval] Claude Code unavailable, using heuristic fallback:`, (err as Error).message);
  ...
  const composite = computeCompositeScore(
    dimensionScores.map(d => ({ score: d.score, weight: d.weight })),
  );

  // Compute domain composite
  const domainComposite = domainDimensionScores.length > 0
    ? computeDomainComposite(
        Object.fromEntries(domainDimensionScores.map(d => [d.key, { score: d.score, weight: d.weight }])),
      )
    : null;
  ...
  let trends: EvalTrend[] | null = null;

  if (prevEval) {
    const prevScores: EvalDimensionScore[] = JSON.parse(prevEval.dimension_scores);
    ...
    trends = dimensionScores.map(d => {
      ...
      return {
        dimension_key: d.key,
        direction: delta > 3 ? 'improving' as const : delta < -3 ? 'declining' as const : 'stable' as const,
        delta,
        previous_score: prev,
  ...
  // Compute risk flags
  const riskFlags = computeRiskFlagsWithHistory(
    dimensionScores.map(d => ({ dimension: d.key, score: d.score })),
    prevEval ? JSON.parse(prevEval.dimension_scores).map((d: EvalDimensionScore) => ({ dimension: d.key, score: d.score })) : null,
    composite,
  );
...
function heuristicEvaluate(
  messages: MessageRow[],
  profile: { weights: Record<BaseDimensionKey, number> },
): { dimensionScores: EvalDimensionScore[]; domainDimensionScores: EvalDimensionScore[]; recommendations: EvalRecommendation[] } {
  const userMessages = messages.filter(m => m.role === 'user');
  const assistantMessages = messages.filter(m => m.role === 'assistant');

  const dimensionScores: EvalDimensionScore[] = BASE_DIMENSION_KEYS.map(key => {
  ...
  const domainWeight = 1 / DOMAIN_DIMENSION_KEYS.length;
  const domainDimensionScores: EvalDimensionScore[] = DOMAIN_DIMENSION_KEYS.map(key => {
  ...
  for (const dim of sorted.slice(0, 3)) {
    ...
    const def = (BASE_DIMENSIONS as Record<string, typeof BASE_DIMENSIONS[BaseDimensionKey]>)[dim.key]
      ?? (DOMAIN_DIMENSIONS as Record<string, typeof DOMAIN_DIMENSIONS[DomainDimensionKey]>)[dim.key];
    if (!def) continue;
...
function avgLen(msgs: MessageRow[]): number {
...
function getToolNames(msgs: MessageRow[]): Set<string> {
  const tools = new Set<string>();
  for (const m of msgs) {
    if (!m.tool_uses) continue;
    try {
      const uses = JSON.parse(m.tool_uses);
      if (Array.isArray(uses)) uses.forEach((u: { name?: string }) => { if (u.name) tools.add(u.name); });
    } catch { /* ignore */ }
  }
  return tools;
}

const HEURISTIC_SCORERS: Record<BaseDimensionKey, (user: MessageRow[], asst: MessageRow[]) => { score: number; reasoning: string }> = {
  task_decomposition(user) {
    ...
    if (ratio > 0.5) { s += 20; r.push('frequent step-by-step'); } else if (ratio > 0.2) { s += 10; r.push('some structure'); }
    if (avgLen(user) > 800 && ratio < 0.2) { s -= 15; r.push('long prompts, no decomposition'); }
  ...
  prompt_specificity(user) {
    ...
    if (al > 500) { s += 15; r.push('detailed'); } else if (al < 50) { s -= 10; r.push('very short'); }
    const code = user.filter(m => /```/.test(m.content ?? '')).length;
    if (code > 0) { s += Math.min(code * 4, 15); r.push(`code examples (${code}x)`); }
    const con = user.reduce((c, m) => c + countPhrases(m.content ?? '', ["don't", 'must', 'should not', 'only', 'ensure']), 0);
    if (con > 3) { s += 12; r.push('constraints'); } else if (con > 0) s += 5;
    return { score: clamp(s), reasoning: r.join('; ') || 'baseline' };
  },
  ...
  iteration_quality(user) {
    ...
    for (let i = 1; i < user.length; i++) {
      ...
      if (a.size > 5 && b.size > 5) {
        ...
        if (inter / Math.max(a.size, b.size) > 0.6) reps++;
      }
    }
    if (reps > 2) { s -= 15; r.push('repetitive'); } else { s += 10; r.push('distinct iterations'); }
    return { score: clamp(s), reasoning: r.join('; ') || 'baseline' };
  },
  strategic_tool_usage(_user, asst) {
    ...
    if (tools.size >= 6) { s += 25; r.push(`${tools.size} tool types`); }
    else if (tools.size >= 3) { s += 15; r.push(`${tools.size} tool types`); }
    else if (tools.size >= 1) { s += 5; r.push(`${tools.size} tool type`); }
    if (tools.has('Read') || tools.has('Grep')) { s += 8; r.push('exploration'); }
    if (tools.has('Edit') || tools.has('Write')) { s += 5; r.push('modification'); }
    if (tools.has('Bash')) { s += 5; r.push('shell'); }
  ...
  context_management(user) {
    ...
    if (first > 500) { s += 15; r.push('strong initial context'); } else if (first < 50) { s -= 5; r.push('minimal context'); }
    const ctx = user.reduce((c, m) => c + countPhrases(m.content ?? '', ['for context', 'background', 'so far', 'to recap', 'previously']), 0);
    if (ctx > 3) { s += 15; r.push('active context mgmt'); } else if (ctx > 0) s += 7;
    return { score: clamp(s), reasoning: r.join('; ') || 'baseline' };
  },
};
...
const DOMAIN_HEURISTIC_SCORERS: Record<DomainDimensionKey, (user: MessageRow[], asst: MessageRow[]) => { score: number; reasoning: string }> = {
  architectural_awareness(user) {
    ...
    if (ac > 5) { s += 25; r.push(`strong architecture awareness (${ac}x)`); }
    else if (ac > 2) { s += 15; r.push('some architecture discussion'); }
    else if (ac > 0) { s += 5; r.push('minimal architecture mentions'); }
    ...
    if (sc > 2) { s += 10; r.push('system-level thinking'); }
  ...
  error_anticipation(user) {
    ...
    if (ec > 5) { s += 30; r.push(`proactive error thinking (${ec}x)`); }
    else if (ec > 2) { s += 15; r.push('some error consideration'); }
    else if (ec > 0) { s += 5; r.push('minimal error awareness'); }
    ...
    if (tc > 2) { s += 10; r.push('tests for error cases'); }
  ...
  technical_vocabulary(user) {
    ...
    if (pc > 6) { s += 25; r.push(`precise vocabulary (${pc} terms)`); }
    else if (pc > 3) { s += 15; r.push('good technical language'); }
    else if (pc > 0) { s += 5; r.push('basic technical terms'); }
    // Check message clarity (longer, more specific messages suggest better vocabulary)
    const al = avgLen(user);
    if (al > 300) { s += 5; r.push('detailed communication'); }
  ...
  dependency_reasoning(user) {
    ...
    if (dc > 5) { s += 25; r.push(`strong dependency awareness (${dc}x)`); }
    else if (dc > 2) { s += 12; r.push('some dependency discussion'); }
    else if (dc > 0) { s += 5; r.push('minimal dependency mentions'); }
    ...
    if (fc > 1) { s += 10; r.push('traces data flow'); }
  ...
  tradeoff_articulation(user) {
    ...
    if (tc > 4) { s += 25; r.push(`explicit tradeoff analysis (${tc}x)`); }
    else if (tc > 1) { s += 12; r.push('some alternatives considered'); }
    else if (tc > 0) { s += 5; r.push('occasional comparison'); }
    ...
    if (rc > 3) { s += 10; r.push('explains reasoning'); }
...
function heuristicTechDetect(messages: MessageRow[]): TechExpertiseEntry[] {
  ...
  const TECH_SIGNALS: Record<string, { patterns: string[]; fileExts?: string[] }> = {
  ...
  const detected: TechExpertiseEntry[] = [];

  for (const [key, { patterns, fileExts }] of Object.entries(TECH_SIGNALS)) {
    let hits = 0;
    for (const p of patterns) {
      if (allText.includes(p)) hits++;
    }
    if (fileExts) {
      for (const ext of fileExts) {
        if (toolText.includes(ext)) hits += 2;
      }
    }
...
function heuristicActivityLog(messages: MessageRow[]): string[] {
  const log: string[] = [];
